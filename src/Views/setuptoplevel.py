...
class SetupTopLevel(CTkToplevel):
    ...
    def create_tab_opcoes_widgets(self, master):
        FilesFrame(
            master,
            self.ctkview.label_settings,
            self.ctkview.button_default_settings,
            self.ctkview.new_db,
            self.ctkview.open_db,
            self.ctkview.export_db,
            fg_color=GRAY,
        ...
        CategorySelectionFrame(
            master,
            self.ctkview.button_default_settings,
            self.ctkview.category_options,
            self.ctkview.category,
            self.category_change_handler,
            fg_color=GRAY,
            **self.ctkview.scrollable_label_settings,
        ...
        GeneralPramsFrame(
            ...
            self.ctkview.var_erase_statement,
            self.ctkview.var_auto_export,
            self.var_aparencia_do_sistema,
            self.ctkview.set_appearance,
            self.var_escala_do_sistema,
            self.ctkview.set_scaling,
            fg_color=GRAY,
    ...
    def create_tab_ajuda_widgets(self, tabela):
        ...
        VersionFrame(tabela, self.ctkview.label_settings, height=32).pack(
    ...
    def category_change_handler(self):
        self.controller.update_user_settings_handler(
            "user_default_category", self.ctkview.category.get()
        )
